...
- 시간 압박으로 *충동 교체매매 위험 ↑* → morning 보다 더 보수적, 핵심 게이트(세후 우월) 충족 시만
- 위기/이벤트 모드 시 단기 헤지가 특히 valid 한 시점 (US 개장 직전 이벤트 리스크 헤지)

**🛑 매도/trim 디시플린** (evening 액션이 *보유 종목 매도·익절·trim* 일 때 — morning 과 동일 잣대):
- 판단 기준은 daily-briefing SKILL.md § 투자 철학(장기 호라이즌) 의 매도 규칙 **그대로**: ① thesis/펀더 훼손, 또는 ② 세후 우월 교체 — 둘 중 하나일 때만.
- **집중도·일시 수익·목표가 근접·52w 고점 근접 단독으로는 trim 권고 X.** 단일 종목 고유위험은 *정보 한 줄*로만 띄우고 매도 verdict 아님 (희석·주의·추가매수 가드).
- carry-over 된 morning 매도 권고도 같은 잣대로 재검증 — ①② 못 넘으면 silent drop. *집중도 높다는 이유만으로 매일 같은 trim 을 반복 권고하지 말 것.*
...
| 조건 | 모드 | 헤더 |
|------|------|------|
| 오늘 *본질 훼손* 위기 시그널 새로 발생 (신용·펀더 — VIX 급등·하락 *단독* 아님; daily Step 3 기준) | **🚨 위기** | `🚨 evening — 위기 신호 발생` |
...
| 비중 (단일 종목 고유위험만) | 단일 고유위험 시 ℹ️ 한 줄 | ❌ skip (큰 변화 시만) |
